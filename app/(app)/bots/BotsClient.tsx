...
/** Tipos base dos seletores */
type Casa = { label: string; slug: string };
type Game = { id: string; label: string; basePath: string; casas: Casa[] };

/** Casa válida especificamente para o bot Aviator (o RobotManager espera isso) */
type CasaSlugAviator = "1win" | "lebull";
const isCasaAviator = (x: string): x is CasaSlugAviator => x === "1win" || x === "lebull";

/** Fonte única de verdade dos jogos/casas exibidos nos selects */
const GAMES: Game[] = [
  {
    id: "aviator",
    label: "Aviator",
...
export default function BotsClient({
  ...
  /** valida game ao montar / quando gameId muda */
  useEffect(() => {
    const exists = GAMES.some((x) => x.id === gameId);
    if (!exists) setGameId("");
  }, [gameId]);
  ...
  /** valida casa ao trocar de jogo ou casa */
  useEffect(() => {
    if (!selectedGame) {
      setCasaSlug("");
      return;
  ...
  useEffect(() => {
    ...
    const next = params.toString() ? `/bots?${params}` : "/bots";
    router.replace(next);
  ...
  return (
    <div className="p-6 max-w-5xl">
      ...
      <section className="mt-8">
        ...
        {/* Só renderiza o RobotManager quando for Aviator + casa válida para Aviator */}
        {selectedGame?.id === "aviator" && isCasaAviator(casaSlug) && (
          <RobotManager
            key={`${selectedGame.id}-${casaSlug}`}
            botId={selectedGame.id}
            casa={casaSlug /* tipado como CasaSlugAviator */}
          />
        )}

        {/* Placeholder para jogos ainda não suportados pelo RobotManager */}
        {selectedGame &&
